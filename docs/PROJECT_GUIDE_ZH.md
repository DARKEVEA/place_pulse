...
### 27.9 RUN_021 Wealthy 五 seed 完成状态

RUN_021 于 2026-08-06 完成 seeds 1103、2207、3319、4421、5527，总墙钟时间为
5.838 小时，五次均正常退出。Mixture 相对 scalar 的改善分别为 0.004857、0.005212、
0.004656、0.005221、0.004759，平均为 0.004941，标准差为 0.000262。只有 2207 和
4421 两个 seed 越过 0.005 门槛；25 个 outer folds 中有 12 个越过门槛。因此 RUN_020
发现的临界预测改善可以重复出现，但不能稳定越过预设的实际意义线。

更重要的是，五个 seed 的最大类别权重均为 98% 左右，按正式的 10% 最小类别权重
门槛计算，有效类别数全部为 1。通过预测门控的 2207 和 4421 分别完成 50 次稳定性
重拟合，ARI 为 0.428 和 0.473，均低于 0.70；可靠排序反转分别为 0/207 和 0/203。
虽然两者的 voter/time auxiliary holdout 通过当前预测门控，但不能补偿有效类别、ARI
和可靠反转三项机制证据的失败。五个 seed 的最终 verdict 因而全部为
`SCALAR_NOT_REJECTED`。

RUN_021 的诚实结论是：Wealthy 存在可重复但极弱的非标量预测结构，平均改善约为
0.49%；该结构不能恢复为两个或更多稳定且拥有不同排序规则的人群。项目保留 scalar
作为 Wealthy 的科学解释模型，不将 mixture 的名义类别解释为社会群体，也不继续扩大
Wealthy seed 数或进入 CUSP 群体机制解释。该结论继续标记为 provisional / exploratory，
不能写成严格 confirmatory 结论。
