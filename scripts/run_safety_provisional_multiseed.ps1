...
$runRoot = Join-Path $repoRoot "artifacts\run_019_safety_provisional_multiseed"
$summaryPath = Join-Path $repoRoot "run_019_safety_provisional_multiseed_summary.json"
...
$seedSummaries = @()
foreach ($seed in $Seeds) {
    $configPath = Join-Path $repoRoot "configs\safety_provisional_seeds\seed_$seed.yaml"
    $artifactRoot = Join-Path $runRoot "seed_$seed"
    $logPath = Join-Path $repoRoot "run_019_safety_seed_$seed.log"
    ...
    $startedAt = Get-Date
    Write-Host "Seed $seed started at $($startedAt.ToString('o'))"
    ...
    if ($exitCode -ne 0) {
        throw "Safety seed $seed failed with exit code $exitCode. See $logPath"
    }
}
